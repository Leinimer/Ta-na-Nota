...
-- Projeto: "Tá na nota"
-- Descrição: 
--   1. Adiciona as colunas `username`, `name` e `email` à tabela `public.profiles`.
--   2. Garante unicidade de `username` (case-insensitive) e `email`.
--   3. Configura o Trigger de sincronização automática com `auth.users` (inclui Google OAuth e Email/Senha).
--   4. Garante Isolamento de Dados por Usuário e Políticas RLS (Row Level Security) em todas as tabelas:
--      - public.profiles
--      - public.nodes
--      - public.notes
--      - public.tags
...
-- Índice único case-insensitive para username (garante que 'Joao' e 'joao' sejam o mesmo login)
CREATE UNIQUE INDEX IF NOT EXISTS idx_profiles_username_lower 
  ON public.profiles (LOWER(username)) 
  WHERE username IS NOT NULL;

-- Índice para buscas rápidas de login por email
CREATE INDEX IF NOT EXISTS idx_profiles_email_lower 
  ON public.profiles (LOWER(email)) 
  WHERE email IS NOT NULL;

-- 2. FUNÇÃO E TRIGGER PARA CRIAR PERFIL AUTOMATICAMENTE AO REGISTRAR NO AUTH
...
BEGIN
  ...
  -- Sanitiza apenas letras, números e underline
  ...
  -- Garante unicidade caso o username gerado já exista
...
-- 3. HABILITAÇÃO DE ROW LEVEL SECURITY (RLS) EM TODAS AS TABELAS
...
-- Permite leitura de username/email para resolução de login no frontend (anon e autenticado)
...
-- 5. POLÍTICAS RLS PARA NODES (Pastas e Nós de Notas)
...
-- 6. POLÍTICAS RLS PARA NOTES (Conteúdo das Notas)
...
-- 8. POLÍTICAS RLS PARA NOTE_TAGS
...
-- 9. POLÍTICAS RLS PARA NOTE_LINKS (Backlinks / Bidirecionais)
...
-- 10. POLÍTICAS RLS PARA ATTACHMENTS (Metadados de Anexos)
...
-- 11. POLÍTICAS RLS PARA O STORAGE BUCKET (attachments)
-- Garante que cada usuário acesse apenas a pasta identificada pelo seu auth.uid()
DO $$
BEGIN
